...
static void User_Task(void *parameters){
	...
	i2c_init();
	i2c_set_speed_mode(I2C_STANDARD_SPEED_MODE);
	uint8_t flag;
	uint32_t slave_register_add = 9;
	uint32_t val = 13;
	uint32_t rval;
	uint8_t send_buf[8];
	uint8_t read_buf[8];
	//i2c_write_reg8(I2C_slave_address, slave_register_add, 0x17);
	for (;;){
		if (task_init){
			printf("\n");
			printf("test\n");
			//flag = i2c_write_reg8(I2C_slave_address, slave_register_add, val&0xff);
			//if(flag){
			//	printf("write failed\n");
			//}
			//else{
				flag = i2c_read_reg8(I2C_slave_address, slave_register_add, &rval);
				printf("flag = %d", flag);
				if(!flag){
